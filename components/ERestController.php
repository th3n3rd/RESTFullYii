...
class ERestController extends CController
{
    ...
    public function onException($event)
    {
        ...
        $errorCode = (!isset($event->exception->statusCode) || is_null($event->exception->statusCode)) ? 500
    ...
    public function filterRestAccessRules($c)
    {
        ...
        Yii::app()->clientScript->enableJavaScript = false; // disable even javascript
        ...
        if (!Yii::app()->user->isGuest && $this->validateAjaxUser($this->action->id)) {
            ...
            if (!(isset($_SERVER['HTTP_X_' . self::APPLICATION_ID . '_USERNAME']) and isset($_SERVER[
            'HTTP_X_' . self::APPLICATION_ID . '_PASSWORD']))
            ) {
                // Error: Unauthorized
            ...
            if ($username != Yii::app()->params['RESTusername']) {
                ...
                if ($password != Yii::app()->params['RESTpassword']) {
                    // Error: Unauthorized
    ...
    public function actionError()
    {
        if ($error = Yii::app()->errorHandler->error) {
            ...
            $this->renderJson(
                array(
                    'success' => false, 'message' => $error['message'], 'data' => array('errorCode' => $error['code'])
                )
            );
        }
    }
    ...
    protected function getNestedRelations()
    {
        ...
        if (!is_array($this->nestedModels) && $this->nestedModels == 'auto') {
            ...
            if (!is_array($this->nestedModels) && $this->nestedModels === false) {
                return $nestedRelations;
            } else {
                if (is_array($this->nestedModels)) {
            ...
            if (is_null($var)) {
                ...
                if (!is_null($var2)) {
                    if ($this->validateSubResource($var, $var2)) {
                        $this->doRestDeleteSubResource($id, $var, $var2);
                    } //Looks like we are trying to delete a subResource
                    else {
    ...
    protected function renderJson($data)
    {
        ...
        $this->render('restfullyii.views.api.output', array('data' => $data));
    }
    ...
    private function setModelAttributes($model, $data)
    {
        foreach ($data as $var => $value) {
            if (($model->hasAttribute($var) || isset($model->metadata->relations[$var]))
                && !in_array(
                    $var, $this->restrictedProperties
                )
            ) {
                $model->$var = $value;
            } else {
                throw new CHttpException(406,
                    'Parameter \'' . $var . '\' is not allowed for model (' . get_class($model) . ')');
        ...
        for ($cnt = 0; $cnt < count($models); $cnt++) {
            ...
            if ($models[$cnt]->validate()) {
                ...
                $message = CJSON::encode(
                    array(
                        'error' => 'Model could not be saved as validation failed.',
    ...
    /**
     * Retrieve Classname even from Namespaced Class.
     *
    ...
    {
        $model = strtolower($this->parseClassName(is_null($model) ? get_class($this->model) : $model));
        $this->renderJson(
            array(
                'success' => true,
                'message' => $message,
    ...
    public function doRestViewSubResource($id, $subResource, $subResourceID = null)
    {
        ...
        if (is_null($subResourceID)) {
            ...
            $this->getModel()->metaData->addRelation(
                $newRelationName, array(
    ...
    public function doRestUpdateSubResource($id, $subResource, $subResourceID)
    {
        ...
        if ($this->saveSubResource($id, $subResourceID, $relationTable, $fks) > 0) {
            $this->renderJson(
                array(
                    'success' => true, 'message' => 'Sub-Resource Added', 'data' => array(
                    $fks[0] => $id,
                    $fks[1] => $subResourceID,
                )
                )
    ...
    {
        return $this->getModel()->dbConnection->commandBuilder->createInsertCommand(
            $relationTable, array(
                $fks[0] => $pk,
                $fks[1] => $fk,
            )
    ...
    public function doRestDeleteSubResource($id, $subResource, $subResourceID)
    {
        ...
        if ($this->getModel()->dbConnection->commandBuilder->createDeleteCommand($relationTable, $criteria)->execute()
            ...
            $data = array(
                'success' => true, 'message' => 'Record Deleted', 'data' => array(
                    $fks[0] => $id,
                    $fks[1] => $subResourceID
                )
            );
